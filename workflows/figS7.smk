...
model_id = 'nn'
train_id = 'main-q0+opt-5-90protocol-q1tr'
...
rule fig_S7:
    ...
    run:
        ...
        for protocol_yaml, (set_type, color) in zip(list(input), set_types_and_colors):
            ...
            ax.plot(protocol.ls[xmin-1:] / 60, np.exp(protocol.interval_logprobs)[xmin-1:], marker='o', ms=3, ls='-', label=set_to_label[set_type].replace('+', '').replace('\n', ' + '), color=color, alpha=.5)
